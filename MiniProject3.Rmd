...
advs_age_stat <- 
  advs %>% 
  filter(SAFFL=="Y" & PARAMCD=="WEIGHT") %>% 
  group_by(PARAMCD, TRTAN, TRTA, SEX) %>% 
  summarize(mean=mean(AGE) %>%  round(digits=1) %>% format(nsmall=1),
    sd=sd(AGE) %>%  round(digits=1) %>% format(nsmall=1),
    med=median(AGE),
    min=min(AGE), 
...
```{r}
varSummaryfcn <- function(.data=NULL, variable=NULL, param=NULL){
  .data %>% 
  filter(SAFFL=="Y" & PARAMCD==param) %>% 
  group_by(PARAMCD, TRTAN, TRTA, SEX) %>% 
...
advs_saf %>% varSummaryfcn(AGE, "WEIGHT")

```
```{r}
advs_age_stat <- 
  advs %>% 
  filter(SAFFL=="Y" & PARAMCD %in% c("HEIGHT", "WEIGHT")) %>% 
  group_by(PARAMCD, TRTAN, TRTA, SEX) %>% 
  summarize(mean=mean(AGE) %>%  round(digits=1) %>% format(nsmall=1),
    sd=sd(AGE) %>%  round(digits=1) %>% format(nsmall=1),
    med=median(AGE),
  ...
  mutate(range_minmax=paste(paste0("(", min), ",", paste0(max, ")"))) %>% 
  ungroup() %>% 
  select("PARAMCD", "TRTA", "SEX", "n", "mean", "med", "sd", "range_minmax") %>% 
  mutate(across(where(is.numeric), .fns = as.character)) %>% 
  pivot_longer(-c("PARAMCD", "TRTA", "SEX"), names_to="category", values_to="values") %>% 
  mutate(SEX = case_when(SEX == "F" ~ "Female", SEX == "M" ~ "Male")) %>% 
  mutate(category = case_when(category == "n" ~ "N",
                              category == "med" ~ "Median", 
                              category == "mean" ~ "Mean", 
                              category == "sd" ~ "Std Dev", 
                              category == "range_minmax" ~ "Range(min,max)")) %>%   pivot_wider(names_from=c("TRTA","SEX"), values_from="values")
