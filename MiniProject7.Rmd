...
RMP7 <- function(adlb_filePath, param=NULL, xvar=NULL, yvar=NULL, xlab=NULL, ylab=NULL){
  adlb_saf <- adlb_filePath %>% 
  ...
  plot <- ALT2 %>%
    ...
    ggplot(mapping=aes(x={{xvar}}, y=as.factor({{yvar}}))) +
    coord_cartesian(xlim=c(0, 150)) +
    stat_dotsinterval(slab_alpha=0) +
    geom_point(data=datap, aes(colour="red")) +
    labs(x = xlab, y = ylab) +
    theme_gray() +
...
RMPCh <- RMP7(read_xpt(inFile),"ALT", LBSTRESN, WEEK, "Alanine Aminotransferase U/L", "Week")
```
