...
ALT2 %>%
  ggplot() +
  geom_bar(mapping = aes(x=as.factor(WEEK), colour=LBNRIND))
```
...
xlabel <- quo_name(plot1b$mapping$x)
ylabel <- quo_name(plot1b$mapping$y)

plot1e <- plot1b +
  labs(x =  ifelse(xlabel=="ADY","Analysis Relative Day", xlabel),
       y =  ifelse(ylabel=="LBSTRESN","Numeric lab results in std units", ylabel))
...
plot2b$mapping
plot2c <- plot2b +
  labs(x = var_label(ALT2[['WEEK']]), 
       y = var_label(ALT2[['LBSTRESN']]))
plot2c
...
RMP6 <- ALT2 %>% 
  ...
  ggplot(mapping=aes(x=LBSTRESN, y=as.factor(WEEK), xmin=uniqueVal(ALT2[['LBSTRESN']][1]))) + 
  coord_cartesian(xlim=c(0, 150)) +
  stat_dotsinterval(slab_alpha=0) +
  geom_point(data=filter(ALT2, LBNRIND %in% c("LOW", "HIGH") & WEEK %in% c(0, 1, 2, 4, 8, 16)), aes(colour="red")) +
  labs(x = 'Alanine Aminotransferase U/L',
       y = 'Week') +
  theme_gray() +
  theme(legend.position="none") +
  draw_label("DRAFT", x=75, y=3, color = "grey", alpha=0.3, size = 50, angle=45, hjust = 0.5, vjust = 0.5) 
RMP6
```
```{r}
RMP6 <- ALT2 %>% 
...
RMP6plot <- ALT2 %>%
  ...
  coord_cartesian(xlim=c(0, 150)) +
  stat_dotsinterval(slab_alpha=0) +
  geom_point(data=RMP6, aes(colour="red")) +
  labs(x = 'Alanine Aminotransferase U/L',
       y = 'Week') +
  theme_gray() +
  theme(legend.position="none") +
