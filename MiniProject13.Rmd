...
adsl_mrg_cnt_fcn <- function(Big_N_Cnt, small_n_cnt, var){
  adsl_mrg_cnt <- small_n_cnt %>%
  left_join(Big_N_Cnt, by = c("TRT01A", "TRT01AN")) %>%
  mutate(perc = round((n / N) * 100, digits = 1),
    perc_char = format(perc, nsmall = 1),
    npct = paste(n, paste0("(", perc_char, ")"))) %>% 
    ungroup() %>% 
    select(TRT01A, {{var}}, npct) %>% pivot_wider(names_from = TRT01A, values_from = npct)
  return(adsl_mrg_cnt)
}

## Calculating count for each category of SEX
small_n_cnt <- small_n_cnt_fcn(SEX)
adsl_mrg_cnt_fcn(Big_N_Cnt, small_n_cnt, SEX) %>% 
  mutate(SEX = recode(SEX, "M" = "Male", "F" = "Female"))

## Calculating count for each category of AGEGR1
small_n_cnt <- small_n_cnt_fcn(AGEGR1)
adsl_mrg_cnt_fcn(Big_N_Cnt, small_n_cnt, AGEGR1)

## Calculating count for each category of RACE
small_n_cnt <- small_n_cnt_fcn(RACE)
adsl_mrg_cnt_fcn(Big_N_Cnt, small_n_cnt, RACE)
```
