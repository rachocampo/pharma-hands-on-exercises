...
```{r}
plot1 <- PR %>%
  ggplot(data = PR, mapping = aes(x=ADY, y=AVAL, group=USUBJID, colour=TRTA, shape=TRTA)) +
  geom_line() + labs(title = "Plot Title",
...
```{r}
plot2 <- plot1 + 
  labs(title = "Individual Profile Plot of Participants",
...
plot3 + 
  stat_summary(data = dataWeeks,
    mapping = aes(x = ADY, group = NULL), fun = median, colour = "red")
